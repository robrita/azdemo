...
def require_api_key(func_to_wrap):  # type: ignore[no-untyped-def]
    ...
    def wrapper(req: func.HttpRequest) -> func.HttpResponse:
        ...
        # Validate API key
        if not provided_api_key:
            logger.warning(f"[{request_id}] Authentication failed: No API key provided in request (method={req.method}, route={req.route_params})")
            return func.HttpResponse(
                body=json.dumps({
        ...
        logger.info(f"[{request_id}] Authentication successful, proceeding to {func_to_wrap.__name__}")  # type: ignore[attr-defined]
...
def _execute_search_query(
    ...
    logger.info(f"[{request_id}] Embedding for '{search_text[:50]}...' generated (dimensions={len(embedding)}, duration={embed_time:.3f}s)")
    ...
    logger.info(f"[{request_id}] Query for '{search_text[:50]}...' returned {len(items)} results (duration={query_time:.3f}s)")
...
def search_cosmosdb(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if not azure_endpoint or not api_key:
            raise ValueError("Missing required environment variables: AZURE_OPENAI_ENDPOINT or AZURE_OPENAI_API_KEY")
        ...
        if not cosmos_endpoint or not cosmos_db_name or not cosmos_container_name:
            raise ValueError("Missing required environment variables: COSMOS_DB_ENDPOINT, COSMOS_DB_DATABASE_NAME, or COSMOS_DB_CONTAINER_NAME")
        ...
        return func.HttpResponse(
            json.dumps({"error": "Invalid request body", "message": "Please provide valid JSON with a 'search' parameter"}),
            mimetype="application/json",
            status_code=400
        )
        ...
        return func.HttpResponse(
            json.dumps({"error": "Initialization error", "message": "Failed to initialize Azure services"}),
            mimetype="application/json",
            status_code=500
        )
    ...
    if not isinstance(search_queries, list):
        ...
        return func.HttpResponse(
            json.dumps({"error": "Validation error", "message": "'search' parameter must be an array of strings"}),
            mimetype="application/json",
            status_code=400
        )
    ...
    if len(search_queries) == 0:
        ...
        return func.HttpResponse(
            json.dumps({"error": "Validation error", "message": "'search' array must contain at least one query"}),
            mimetype="application/json",
            status_code=400
        )
    ...
    try:
        ...
        logger.info(f"[{request_id}] Search completed: {len(all_results)} total results, {len(unique_results)} unique results")
        ...
        total_time = time.time() - start_time
        logger.error(f"[{request_id}] Search operation failed after {total_time:.3f}s: {type(e).__name__}: {str(e)}", exc_info=True)
        return func.HttpResponse(
            body=json.dumps({
