...
### Azure Functions Serverless Compatibility

**CRITICAL**: ALL code must be compatible with Azure Functions serverless environment:

1. **NEVER write to the file system** except for `/tmp` directory (which is ephemeral)
2. **Process all data in-memory** using streams, buffers, and memory-based operations
3. **No persistent local state** - use Azure Storage, Cosmos DB, or other cloud services for persistence
4. **Stateless operations** - each function invocation should be independent
5. **Use streaming for large files** (e.g., `fitz.open(stream=bytes)` instead of file paths)
6. **Avoid file-based operations** like `open()`, `Path().write()`, `os.makedirs()`, etc.
...
1. **ALWAYS** check for Ruff errors immediately after completing the implementation
2. **ALWAYS** fix all Ruff errors before considering the task complete
...
5. Only in extremely rare cases where a Ruff error is a false positive should you consider suppression
6. Ensure no warnings or errors remain in the code

#### Common Scenarios & Solutions

**Scenario: Azure Functions Request Headers Type Issues**
When extracting headers from `func.HttpRequest`, Ruff may report "Type is partially unknown" errors because `req.headers` lacks proper type information in the Azure Functions SDK.
...
When extracting `Content-Length` from request headers, Ruff reports type errors because the header value is unknown and the default parameter type doesn't match.
...
# Now Ruff knows: headers.get() returns str | None → int() receives str → result is int
```
...
When accessing headers from Azure SDK exceptions (e.g., `CosmosHttpResponseError`), Ruff reports "Type is partially unknown" because the SDK doesn't provide complete type information for exception attributes.
...
# Now Ruff is satisfied - getattr returns Any which can be cast properly
```

**Key principle**: For SDK exception objects with partially unknown attributes, use `getattr()` to extract the attribute into a variable with `Any` type, then cast that variable to the desired type. This provides a clean type boundary that Ruff can verify.

**Scenario: Dictionary Initialization with Partially Unknown Types**
When initializing dictionaries with mixed value types, Ruff may infer partially unknown types if it cannot determine the complete type structure.
...
# Now Ruff knows the complete type structure
```

**Key principle**: When creating dictionaries with mixed or nested value types, add explicit type annotations to help Ruff understand the intended structure, especially when using `Any` for flexible value types.
...
- ✅ Confirmed no Ruff errors after cleanup
...
- [ ] **Code is Azure Functions serverless-compatible (NO file system writes, all in-memory processing)**
...
4. Verified no Ruff errors throughout
